...
event.listen(
    Base.metadata, "before_create", DDL("CREATE SCHEMA IF NOT EXISTS other_schema")
)
...
class ReflectionDistKey(Base):
    ...
    __table_args__ = {"redshift_diststyle": "KEY", "redshift_distkey": "col1"}
...
class ReflectionUniqueConstraint(Base):
    ...
    col1 = sa.Column(sa.Integer(), primary_key=True)
    col2 = sa.Column(sa.Integer())
    __table_args__ = (sa.UniqueConstraint(col1, col2), {"redshift_diststyle": "EVEN"})
...
class ReflectionNamedPrimaryKeyConstraint(Base):
    ...
    __table_args__ = (
        sa.PrimaryKeyConstraint(
            col1, col2, name="reflection_named_pk_constraint__pkey"
        ),
        {"redshift_diststyle": "EVEN"},
    )
...
class ReflectionForeignKeyConstraint(Base):
    ...
    col2 = sa.Column(sa.Integer())
    __table_args__ = {"redshift_diststyle": "EVEN"}
...
class ReflectionNamedForeignKeyConstraint(Base):
    ...
    col1 = sa.Column(
        sa.Integer(),
        sa.ForeignKey(
            "reflection_unique_constraint.col1",
...
class ReflectionDelimitedTableName(Base):
    ...
    __table_args__ = {"redshift_diststyle": "EVEN", "schema": "other_schema"}


class ReflectionDelimitedTableNoSchema(Base):
    __tablename__ = "this.table"
    col1 = sa.Column("id", sa.Integer(), primary_key=True)
    __table_args__ = {"redshift_diststyle": "EVEN", "schema": None}
